...
interface ShareSummaryWithGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  summaryId: string;
  summaryTitle: string;
...
export function ShareSummaryWithGroupModal({
  isOpen,
  onClose,
  summaryId,
  summaryTitle,
  currentGroupId,
  onSuccess,
}: ShareSummaryWithGroupModalProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      await summariesService.shareWithGroup(summaryId, selectedGroupId);
      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.response?.data?.message || 'Error sharing summary');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUnshare = async () => {
    if (!window.confirm('Are you sure you want to stop sharing this summary with the group?')) {
      return;
    }
    ...
    try {
      setLoading(true);
      setError(null);
      await summariesService.unshareFromGroup(summaryId);
      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.response?.data?.message || 'Error unsharing summary');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Share Summary with Group</h2>
          ...
          {loadingGroups ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-400 text-xs mt-2">
                Create a group first to share summaries.
              </p>
            </div>
          ) : (
            <>
              ...
              {currentGroupId && (
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <p className="text-sm text-yellow-800">
                    This summary is currently shared with a group.
                  </p>
                </div>
              )}
            </>
