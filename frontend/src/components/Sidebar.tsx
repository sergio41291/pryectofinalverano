import { Home as HomeIcon, FolderOpen, BrainCircuit, Users, Settings, LogOut, BookOpen } from 'lucide-react';

// Añadimos 'setSeccion' para que la Sidebar pueda cambiar la vista
    ...
    <aside className="fixed top-0 left-0 z-50 flex flex-col w-64 h-screen bg-white border-r border-gray-200">
      ...
      <nav className="flex-1 p-4 space-y-2">
        {menuItems.map((item) => (
          <button 
            key={item.id}
            onClick={() => setSeccion(item.id)} // Al hacer clic, cambia la sección
      ...
      <div className="p-4 space-y-1 border-t border-gray-100">
        ...
        <button onClick={() => window.location.reload()} className="flex items-center w-full gap-3 px-4 py-3 text-red-500 transition-all hover:bg-red-50 rounded-xl">
          <LogOut size={20} />
